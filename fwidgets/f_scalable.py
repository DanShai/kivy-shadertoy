...
class ScalableBehaviour(object):
    initial_font_size = NumericProperty(32)
    fscale = NumericProperty(.9)
    min_font_size = NumericProperty(32)
    max_font_size = NumericProperty(32)
    ...
    def calculate_font(self, *largs):
        try:
            if self.texture_size:
                factor = [self.font_size / self.texture_size[0], self.font_size / self.texture_size[1]]
