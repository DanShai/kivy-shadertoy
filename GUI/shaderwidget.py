...
class ShaderWidget(Widget):
    ...
    iChannelResolution = ListProperty(
    ...
    def __init__(self, **kwargs):
        ...
        # print shader_header
        self.shader_header = shader_header + \
            open('glsl_headers/shader_header.frag', 'r').read()
        self.shader_source = open('glsl/blank.frag', 'r').read()
        self.shader_footer = open(
            'glsl_headers/shader_footer.frag', 'r').read()
        self.definition_template = open(
            'glsl_headers/definition.frag', 'r').read()
    ...
    def save_file(self, pp, fsp, scode, *args):
        ...
        with open('glsl/' + self.new_name, 'w') as file:
            file.write(txt)
        pp.dismiss()
        fsp.values = list(map(str, sorted(listdir('glsl'))))
    ...
    def launch_popup(self, f_spinner, scode):
        ...
        box = FBoxLayout(orientation='vertical', spacing='24dp', bg_color=[
                         'Brown', '500'], size_hint=(1., 1.), padding='20dp')
        ...
        popup = Popup(title="Save/Rename",  content=box, size_hint=(.6, .6),
                      title_size='64dp', title_align='center', title_font=font_name)
