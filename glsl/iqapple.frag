...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // camera
    vec3 ro = 2.5*normalize(vec3(cos(0.2*iTime),1.9+0.3*cos(iTime*.11),
								  sin(0.2*iTime)));
    mat3 mcam = doCamera(ro , vec3(0.0,.5,0.0) );
	vec3 rd = normalize(mcam * vec3(p,1.5) );
